...
# Then rebuild NAS first (adds NFS export), then arch.
{
  ...
  # ── GenAI shared configuration ─────────────────────────────────────
  # Imported from standalone file — the same config is used by flake.nix
  # for Docker image builds in CI without evaluating the full NixOS system.
