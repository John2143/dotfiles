...
async def init_agent_state_activity(init_arg: dict) -> dict:
    ...
    system_prompt = (
        ...
        "If you upscaled, review the upscaled result before concluding."
    )
    if camera_desc:
        system_prompt += f"\n\n{camera_desc}"
    if label_hint:
...
async def tool_upscale_activity(arg: dict) -> dict:
    ...
    if image_bytes is None:
        ...
        _atomic_write(msg_path, state)
        return {"source": source, "error": "source_not_found"}
    ...
    if max(src_img.size) > max_input_px:
        ...
        return {"source": source, "error": "input_too_large",
                "width": src_img.width, "height": src_img.height}
    ...
    if result_bytes is None:
        ...
        _atomic_write(msg_path, state)
        return {"source": source, "error": "api_unavailable"}
